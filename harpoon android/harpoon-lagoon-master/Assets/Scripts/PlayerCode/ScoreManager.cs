...
public class ScoreManager : MonoBehaviour {
	...
	public void ResetScore(bool resetDayAlso = true) {
        ...
        if (MenuStateMachine.levSequence == 0)
        {
            Debug.Log("Total : " + total);
            Debug.Log("High7 : " + high7);
            if (total > high7)
            {

                PlayerPrefs.SetInt("7", total);
                PlayGamesScript.AddScoreToLeaderboard(GPGSIds.leaderboard_7_day_score, total);
                //GC_Calls.ReportScore(total, "com.clownboxgames.harpoonlagoon.7daybs");
            }
        }
        else if (MenuStateMachine.levSequence == 1)
        {
            if (total > high14)
            {
                PlayerPrefs.SetInt("14", total);
                PlayGamesScript.AddScoreToLeaderboard(GPGSIds.leaderboard_14_day_score, total);
                //GC_Calls.ReportScore(total, "com.clownboxgames.harpoonlagoon.14daybs");
            }
        }
        else if (MenuStateMachine.levSequence == 2)
        {
            if (total > high28)
            {
                PlayerPrefs.SetInt("28", total);
                PlayGamesScript.AddScoreToLeaderboard(GPGSIds.leaderboard_28_day_score, total);
                //GC_Calls.ReportScore(total, "com.clownboxgames.harpoonlagoon.28daybs");
            }
        }
    ...
    void Start()
    {
        high7 = PlayerPrefs.GetInt("7");
        high14 = PlayerPrefs.GetInt("14");
        high28 = PlayerPrefs.GetInt("28");
